...
export const createIframeLink = (videoId: string) =>
  `https://iframe.mediadelivery.net/embed/423572/${videoId}?autoplay=true&preload=true`

/* eslint-disable @typescript-eslint/no-explicit-any */
export const doesTitleMatch = (video: any, searchQuery: string) =>
  ilike(
    sql`REPLACE(REPLACE(REPLACE(LOWER(${video.title}), '-', ''), '.', ''), ' ', '')`,
